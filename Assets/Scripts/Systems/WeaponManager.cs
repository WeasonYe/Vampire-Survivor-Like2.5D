...
public class WeaponManager : Singleton<WeaponManager>
{
    ...
    [Tooltip("武器预制体映射")]
    [SerializeField]
    private List<WeaponPrefabMapping> weaponPrefabMappings = new List<WeaponPrefabMapping>();
    ...
    public WeaponBase AddWeapon(AttackStrategySO strategy, int startLevel = 1)
    {
        ...
        // 选择合适的武器预制体（优先级：策略自带 > 映射 > 默认）
        GameObject weaponPrefab = null;

        // 1. 优先使用策略中指定的预制体
        if (strategy != null && strategy.weaponPrefab != null)
        {
            weaponPrefab = strategy.weaponPrefab;
        }
        else
        {
            // 2. 使用映射中的预制体
            weaponPrefab = GetAppropriateWeaponPrefab(strategy);
        }
    ...
    private GameObject GetAppropriateWeaponPrefab(AttackStrategySO strategy)
    {
        // 查找策略对应的预制体
        foreach (var mapping in weaponPrefabMappings)
        {
            if (mapping.strategyType == strategy.GetType())
            {
...
{
    [Tooltip("策略类型")]
    public System.Type strategyType;
